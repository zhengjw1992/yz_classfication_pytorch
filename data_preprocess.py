...
def read_data(train_directory,dir2label_dict):
    filename_list = []
    label_list = []
    sub_directory_list = os.listdir(train_directory)   # train_data or validation_data
    # 可以考虑将sub_directory_list写死 以指定读取的label顺序
    for sub_directory in sub_directory_list:
        sub_directory_path = os.path.join(train_directory,sub_directory)  # 每一个子类别
